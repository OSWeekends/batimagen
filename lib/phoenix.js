...
function validateParams (params) {
  ...
  properties.forEach(property => {
    if (!params[property])
      throw `[error][phoenix process] Missing ${property} property in configuration object!`
  })

  supportedActions.forEach(action => {
    if (!params.action === action)
      throw `[error][phoenix process] Not supported action: ${action}!`
  })
...
function generateCmd (params) {
  ...
  return `${phoenixPath} -f ${params.file} -o ${
    params.output ? params.output : ''
  } ${params.display ? '-d' : ''} -${params.action}`
}
...
function phoenixHandler (params, cb) {
  if (validateParams(params)) {
    exec(generateCmd(params), function (err, stdout, stderr) {
      ...
      if (!err) {
        console.log(
          `[info][phoenix process] action ${params.action} over ${
            params.file
          }: done`
        )
        cb(null, output)
        ...
        console.log(
          `[error][phoenix process] action ${params.action} over ${
            params.file
          } error:\n ${err}`
        )
        cb(err)
