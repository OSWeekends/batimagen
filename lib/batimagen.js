...
const { imageValidator, fileDigestion } = require('./digester')
const { reports } = require('../lib/database')
const config = require('../config')

function processFileDetails (fileInfo, req) {
  const tasks = [
    { task: metaAnalizer({ name: fileInfo.filename }, req), role: 'metadata' }
  ]
  ...
  if (imageValidator(fileInfo.extension) && config.thirdPartiesEnabled) {
    if (config.thirdParties.googleVision) {
      ...
      tasks.push({
        task: visionAnalysis.fullAnalysis(fileInfo.fullpath),
        role: 'vision'
      })
    }
  }
