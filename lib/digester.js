...
function digestExternalFile (req, data){
    return new Promise((resolve, reject) => {
        ...
        client.get(req.body.url, response => {
            data.filename = `${data.uuid}.${mime.extension(response.headers["content-type"])}`;
            data.fullpath = `${paths.temp}/${data.filename}`;
...
function fileDigestion (req, data){
    return req.files && req.files.file ? digestFormFile (req, data) : digestExternalFile (req, data);
}
