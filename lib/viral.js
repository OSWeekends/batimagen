...
con.setKey(token.virustotal)
con.setDelay(15000)

function fileAnalysis (fileData, cb) {
  return new Promise((resolve, reject) => {
    con.submitFileForAnalysis(
      fileData.fullpath,
      fileData.mimetype,
      fs.readFileSync(fileData.fullpath),
      data => {
...
function virusAnalizer (fileData) {
  return fileData.source === 'url'
    ? urlAnalysis(fileData)
