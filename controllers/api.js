...
{services, reports} = require('../lib/database'),
...
function createReport (req, res) {
  if(!fileValidation(req)) {
    return res.status(400).json({ msg: 'No files were attached in the request.'});
  }
  const uuid = uuidv4()
  processFile(req, {uuid})
  res.json({uuid, msg: `Process has started. Check /api/v1/reports/${uuid} for more info...`})
}
...
module.exports = {listServices, listReports, deleteReport, getReport, createReport}