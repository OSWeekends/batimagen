...
function remove (req, res) {
  ...
  if(!uuid) {
    return res.status(400).render('error', { error: "Bas request! Missing UUID!"})
  }

  const report = reports.getOne(uuid);
  if(!report) {
    return res.status(404).render('error', { error: "This report does not exist!"})
  } 
  
  if(!report.isReady){
    return res.status(400).render('error', { error: "This report wasn't generated yet!"})
  } 
